...
/**
 * @author: wuguangyuan
 * @create-date: 2022/8/13 23:18
 */
public class JdbcConn {
    private DriverManager DriverHanager;
    ...
    public void connect02() throws ClassNotFoundException, IllegalAccessException, InstantiationException, SQLException {
        ...
        String url = "jdbc:mysql://localhost:3306/RUNOOB";//将用户名和密码放入到Properties 对象
        Properties properties = new Properties();
        //说明user和 password 是规定好，后面的值根据实际情况写
        properties.setProperty("user", "root");//用户
        properties.setProperty("password", "root");//密码
        Connection connect = driver.connect(url, properties);
    ...
    public void connect04() throws ClassNotFoundException, SQLException {
        //使用反射加载了 Driver类//在加载Driver类时，完成注册
        /*
        源码:1。静态代码块，在类加载时，会执行一次。
        2.DriverManager.registerDriver(new Driver());3．因此注册driver的工作已经完成
        static {
            try {
        ...
        //jar包下META-INF\servicesVjava .sqI.Driver文本中的类名称去注册
        //3.建议还是写上 CLass . forName("com.mysql.jdbc.Driver"),更加明确

        Class.forName("com.mysql.jdbc.Driver");
        //创建url 和user和 password
        String url = "jdbc:mysql://localhost:3306/RUNOOB";//将用户名和密码放入到Properties 对象
        String user = "root";
        String password = "root";
        Connection connection = DriverManager.getConnection(url,user,password);
    ...
    public void connect05() throws IOException, ClassNotFoundException, SQLException {
        ...
        Properties properties = new Properties();
        properties.load(new FileInputStream("chapter25\\src\\mysql.properties"));//获取相关的值

        //获取相关的值
        String user = properties.getProperty( "user");
        String password = properties.getProperty("password");String driver = properties.getProperty("driver");
